...
class ZigZagAviary(BaseSingleAgentAviary):
    """Single agent RL problem: move zig-zag at position."""
    ...
    def _computeReward(self):
        ...
        # return -2 / (10*state[2] + 10*state[1])
        return -(1-norm_ep_time) * np.linalg.norm(np.array([0, 0.5, 1])-state[0:3])**2 + -norm_ep_time * np.linalg.norm(np.array([0, 0, 1])-state[0:3])**2 - norm_ep_time*np.linalg.norm(state[3:6])**2 #-1 * np.linalg.norm(np.array([0, 0, 1])-state[0:3])**2
